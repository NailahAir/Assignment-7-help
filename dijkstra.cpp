...
*/
void propergateSignal( Graph& g, int x, EventQueue& q);
...
*/
void deletePriorityQueue(EventQueue& q);

/*
prints the graph g from starting point to the finish point d
*/
void showPath(Graph g, int start, int finish);
...
void propergateSignal( Graph& g, int x, EventQueue& q)
{
	...
	while( temp != NULL )
	{
		if( g.arrayV[temp->v].t == -1 )
		{
			...
			sendSignal( temp->u, temp->v, g.arrayV[x].t+temp->w, q);
			if(traceEnable == 1)
			{
...
void showPath(Graph g, int start, int finish)
{
	...
	{
		showPath( g, start, g.arrayV[finish].f );
		printf("-> %d ", finish );
	}
}
